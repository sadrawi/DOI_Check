"""
DOI checker for a Word reference list (Crossref).

Give it a .docx: it finds the References section, checks every entry, and writes
a marked-up copy where each reference is highlighted by status with an inline
note carrying the correct DOI. A plain pasted list works too.

Two checks per reference:
  1. BACKWARD - if the entry already carries a DOI, resolve it at Crossref and
     see which paper it actually points to. Catches the dangerous case of a real
     DOI registered to a different article.
  2. FORWARD  - send the whole reference string to Crossref bibliographic search
     and take the best-matching record's DOI as the suggested correct one.

Match confidence = how much of the Crossref record (title words, lead author
surname, year) is actually present in your entry. Nothing is trusted unless it
resolves.

Colab-compatible: paste into one cell and run.
"""
...
def check_all(references):
    """references = list of (text, [paragraph indices])."""
    rows = []
    for i, entry in enumerate(references):
        text = entry[0]
        print("[" + str(i + 1) + "/" + str(len(references)) + "] " + text[:70] + "...")
        row = check_reference(text)
        row["n"] = i + 1
        row["paragraphs"] = entry[1]
        rows.append(row)
        print("     -> " + row["status"] + "  " + row["correct_doi"])
...
def upload_in_colab():
    """Prompt for a .docx upload when running in Colab. Returns the filename."""
...
def mark_docx(document, results, out_path):
    """Highlight each reference paragraph by status and append an inline note."""
    paragraphs = document.paragraphs
...
def run():
    ...
    if USE_DOCX:
        path = DOCX_PATH
        if path == "":
            path = upload_in_colab()
        document, references = read_references_from_docx(path)
        stem = os.path.splitext(os.path.basename(path))[0]
        out_path = stem + "_checked.docx"
        print("Found " + str(len(references)) + " references in " + os.path.basename(path) + "\n")
    else:
        references = load_references_from_text(REFERENCES_TEXT)
        print("Checking " + str(len(references)) + " pasted references\n")

    results = check_all(references)
    report(results)

    results.drop(columns=["paragraphs"]).to_csv(CSV_OUT, index=False)
    print("\nSaved: " + CSV_OUT)

    if document is not None:
        mark_docx(document, results, out_path)

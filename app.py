...
def cached_check(reference, email, threshold):
    core.EMAIL = email
    core.SCORE_THRESHOLD = threshold
    return core.check_reference(reference)


def check_with_cache(references, email, threshold, progress_bar, status_line):
    rows = []
    for i, entry in enumerate(references):
        status_line.write("Checking " + str(i + 1) + " of " + str(len(references)) + ": " + entry[0][:80] + "...")
        row = cached_check(entry[0], email, threshold)
        row = dict(row)
        row["n"] = i + 1
...
email = st.sidebar.text_input(
    ...
    value=st.secrets.get("crossref_email", "") if hasattr(st, "secrets") else "",
    help="Puts requests in Crossref's polite pool. Shared cloud IPs get throttled "
         "more often, so this helps on longer lists.",
)

threshold = st.sidebar.slider(
    "Match threshold", 0.40, 0.90, 0.65, 0.05,
    help="Below this, a suggested DOI is reported but flagged unconfirmed.",
)

core.HIGHLIGHT_OK = st.sidebar.checkbox("Highlight verified entries too", value=True)
core.PRESERVE_EXISTING_HIGHLIGHT = st.sidebar.checkbox(
    "Keep existing highlighting", value=True,
    help="Leaves a copyeditor's yellow marks intact instead of overwriting them.",
)

st.sidebar.caption("Reference sections are found by heading: " + ", ".join(core.REF_HEADINGS[:4]) + ", ...")
...
def render(results, docx_bytes, filename):
    ...
    table = results[["n", "status", "given_doi", "correct_doi", "score", "note"]]
...
if run_docx and uploaded is not None:
    ...
    bar = st.progress(0.0)
    line = st.empty()
    results = check_with_cache(references, email, threshold, bar, line)
    bar.empty()
